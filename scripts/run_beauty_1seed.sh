...
# 说明：
# - 这个脚本只用于快速验证，默认只跑 1 个 seed
# - 官方复现结果仍然以 run_beauty_5seeds.sh 为准
...
echo "single-seed quick run, seed=$SEED"

# 1) baseline：普通 SASRec
python run.py seed=$SEED $COMMON_ARGS

# 2) content initialization：内容向量初始化
python run.py seed=$SEED $COMMON_ARGS use_pretrained_item_embeddings=True

# 3) trainable delta：论文核心方法
python run.py seed=$SEED $COMMON_ARGS use_pretrained_item_embeddings=True train_delta=True
